...
from IPPy import operators, solvers, stabilizers
...
Psi_k = stabilizers.Tik_CGLS_stabilizer(
    kernel, setup["is"]["reg_param"], k=setup["is"]["n_iter"]
)
...
if not args.test:
    ...
    for i in range(len(train_data)):
        if (i + 1) % 100 == 0:
            print(
                f"Done {i+1} image(s). Time passed: {time.time() - start_time:0.4f}s. Avg. SSIM: {tot_ssim/(i+1)}."
            )
